...
import com.gengptx.sever.gengptx.generator.GPTGenerator;
import com.gengptx.sever.gengptx.generator.SynthGeneratorX;
import com.gengptx.sever.gengptx.generator.XMLWriter;
import com.gengptx.sever.gengptx.structure.GoalNode;
import com.gengptx.sever.gengptx.structure.Literal;
...
public class GeneratorXService {
    public JSONObject generateGpt(HttpServletRequest request,int seed, int sy_depth, int num_tree, int sy_num_goal, int sy_num_plan, int sy_num_action, int sy_num_var,
                                  int sy_num_selected, double sy_prob_leaf, int sy_num_group) {
//        check the value
        ...
        if(sy_num_group >= 100){
            jsonObject.put("msg","Too many groups, please reduce the number of groups");
            return jsonObject;
        }
         ...
         * the generator
         */
        GPTGenerator gen = new SynthGeneratorX(sy_depth, num_tree, sy_num_goal, sy_num_plan, sy_num_action, sy_num_var, sy_num_selected, sy_prob_leaf, sy_num_group);
    ...
    private static JSONObject JSONConverter(JSONObject obj) {
        ...
        if (obj.containsKey("goalConds")) {
            ...
            for (int i = 0; i < goalConds.size(); i++) {
                ...
                conds.put("goal-conditions", cond.get("group")+"("+cond.get("id")+")");
            }
        }
        if (obj.containsKey("pre")) {
            ...
            for (int i = 0; i < preConds.size(); i++) {
                ...
                conds.put("pre-condition", cond.get("group")+"("+cond.get("id")+")");
            }
        }
        if (obj.containsKey("post")) {
            ...
            for (int i = 0; i < postConds.size(); i++) {
                ...
                conds.put("post-condition", cond.get("group")+"("+cond.get("id")+")");
            }
        }
